...
func NewUDPHandler(timeout time.Duration, cc func(fd int)) core.UDPConnHandler {
	return &udpHandler{
		timeout:        timeout,
...
func (h *udpHandler) fetchUDPInput(conn core.UDPConn, pc *net.UDPConn) {
	...
	for {
		...
		if err != nil {
			fmt.Printf("failed to read UDP data from remote: %v\n", err)
			return
		}
...
func (h *udpHandler) Connect(conn core.UDPConn, target *net.UDPAddr) error {
	...
	tgtAddr, _ := net.ResolveUDPAddr("udp", target.String())
	h.Lock()
...
func FDFromUDPConn(conn net.UDPConn) int {
	cn := reflect.Indirect(reflect.ValueOf(conn)).FieldByName("conn")
	fd := cn.FieldByName("fd")
